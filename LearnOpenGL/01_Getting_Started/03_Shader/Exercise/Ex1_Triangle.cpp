...
namespace mogl {
	...
	namespace ch13ex1 {
		...
		int Triangle::Run(int argc, char *argv[]) {
			glfwInit();
			...
			///![1] 构建GPU程序
			ShaderProgram shader_program("./Shader/0103Ex01_Triangle.vs",
				"./Shader/0103Ex01_Triangle.fs");
			if (!shader_program.CompileAndLink()) {
				return -1;
			}

			shader_program.Use();
			shader_program.SetVec4("outColor", 0.0f, 0.5f, 0.0f, 1.0f);
